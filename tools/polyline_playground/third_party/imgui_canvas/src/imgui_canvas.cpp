...
{
    return CalcViewRect(view, m_WidgetSize);
}

ImRect ImGuiEx::Canvas::CalcViewRect(const CanvasView& view, const ImVec2& widgetSize) const
{
    ImRect result;
    result.Min = ImVec2(-view.Origin.x, -view.Origin.y) * view.InvScale;
    result.Max = (widgetSize - view.Origin) * view.InvScale;
    return result;
}
