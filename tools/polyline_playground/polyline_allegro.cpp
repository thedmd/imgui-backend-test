...
constexpr auto ALLEGRO_PI = IM_PI;

constexpr auto ALLEGRO_VERTEX_CACHE_SIZE = 1024;
...
struct ALLEGRO_PRIM_VERTEX_CACHE
{
    ALLEGRO_PRIM_VERTEX_CACHE_TYPE type;

    ImVec2 vertices[ALLEGRO_VERTEX_CACHE_SIZE];
    int size = 0;
...
void _al_prim_cache_init(ALLEGRO_PRIM_VERTEX_CACHE* cache, ALLEGRO_PRIM_VERTEX_CACHE_TYPE type, ALLEGRO_COLOR color)
{
    cache->type      = type;
    cache->color     = color;
}
...
void _al_prim_cache_flush(ALLEGRO_PRIM_VERTEX_CACHE* cache)
{
    ...
    cache->draw_list->PrimReserve(cache->size, cache->size);

    for (int i = 0; i < cache->size; ++i)
        cache->draw_list->PrimVtx(cache->vertices[i], cache->uv, cache->color);
...
{
    if (cache->type != ALLEGRO_PRIM_VERTEX_CACHE_TRIANGLE)
        return;

    if (cache->size >= (ALLEGRO_VERTEX_CACHE_SIZE - 3))
        _al_prim_cache_flush(cache);

    cache->vertices[cache->size + 0] = ImVec2(v0[0], v0[1]);
    cache->vertices[cache->size + 1] = ImVec2(v1[0], v1[1]);
    cache->vertices[cache->size + 2] = ImVec2(v2[0], v2[1]);
...
{
    if (cache->type != ALLEGRO_PRIM_VERTEX_CACHE_LINE_STRIP)
        return;

    //cache->draw_list->PrimReserve(1, 1);
    //cache->draw_list->PrimVtx(ImVec2(v[0], v[1]), cache->uv, cache->color);
    ...
    ALLEGRO_COLOR color, float thickness, float miter_limit)
{
    ...
    cache.draw_list = draw_list;
    cache.uv        = ImGui::GetFontTexUvWhitePixel();
