...
#define IM_POLYLINE_TRIANGLE(N, A, B, C)                        \
    ...
    const auto idx_base_start = draw_list->_VtxCurrentIdx;
    auto       idx_base       = idx_base_start;
    ...
    for (int i = closed ? 0 : 1; i < count; ++i)
    {
        ...
        const bool  limit_miter        = miter_distance_sqr > miter_distance_limit_sqr;
        ...
        const int   ideal_join_type    = !closed && (i == count - 1) ? Butt : (limit_miter || join == ImDrawFlags_JoinBevel ? Bevel : Miter);
        ...
        switch (join_type)
        {
            case Butt:      idx_base += 4; break;
            case ThickButt: idx_base += 6; break;
            case Bevel:     idx_base += 3; break;
            case Miter:     idx_base += 2; break;
        }
